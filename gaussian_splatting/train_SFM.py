...
def initialize_gaussians_from_points3d(filepath, sh_degree, device='cuda'):
    ...
    gaussians._features_dc = colors_tensor
    gaussians._features_rest = torch.zeros(
        (n_points, 3 * (sh_degree + 1)**2 - 3),
        dtype=torch.float32,
        device=device
    )
    gaussians._scaling = torch.log(
        torch.tensor(np.random.uniform(0.1, 0.5, (n_points, 3)), dtype=torch.float32, device=device)
    )
    gaussians._rotation = torch.nn.functional.normalize(
        torch.randn((n_points, 4), dtype=torch.float32, device=device), dim=1
    )
    gaussians._opacity = torch.nn.Parameter(torch.ones((n_points, 1), dtype=torch.float32, device=device) * 0.8)
    gaussians._features_dc.data.clamp_(0.0, 1.0)
    gaussians._scaling.data = torch.clamp(
        gaussians._scaling.data,
        min=math.log(0.01),  
        max=math.log(10.0)  
    )
    gaussians._opacity.data = torch.clamp(
        gaussians._opacity.data,
        min=0.01,  
        max=0.99  
    )
    print(f"Initialized Gaussian Colors (DC): {gaussians._features_dc.min()} - {gaussians._features_dc.max()}")
    print(f"Initialized Opacity: {gaussians._opacity.min()} - {gaussians._opacity.max()}")
    print(f"Initialized Scaling: {gaussians._scaling.exp().min()} - {gaussians._scaling.exp().max()}")
    print(f"Initialized from SfM points: {filepath}")
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        rand_idx = randint(0, total_cameras - 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7_000, 30_000, 50_000, 100_000, 300_000, 1000_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[7_000, 30_000, 50_000, 100_000, 300_000, 1000_000])

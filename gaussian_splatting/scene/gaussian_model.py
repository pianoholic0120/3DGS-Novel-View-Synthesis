...
class GaussianModel:
    ...
    def parameters(self):
        ...
        return [p for p in self._parameters if p is not None and p.requires_grad]
    ...
    def load_state_dict(self, state_dict):
        ...
        self._exposure = state_dict["_exposure"]
        if self.optimizer and state_dict["optimizer_state"]:
    ...
    def training_setup(self, training_args):
        self.optimizer = torch.optim.Adam([self._xyz, self._features_dc, self._features_rest], lr=0.001) 
        ...
        if not hasattr(self, '_exposure'):
            raise AttributeError("'GaussianModel' object has no attribute '_exposure'. Ensure _exposure is initialized.")
